    ...
    switch ($action) {
        case 'toggle_robot':
            ...
            if ($newStatus === 'active' && !$otSetupCompleted) {
                $message = __('dashboard_setup_first');
                $messageType = 'danger';
                break;
            }
            ...
            $message = $newStatus === 'active' ? __('dashboard_robot_activated') : __('dashboard_robot_paused');
        ...
        case 'save_settings':
            ...
            $robotSettings = getRobotSettings($_SESSION['user_id']);
            $message = __('dashboard_settings_saved');
            $messageType = 'success';
            break;
        ...
        case 'save_targets':
            ...
            $robotSettings = getRobotSettings($_SESSION['user_id']);
            $message = __('dashboard_targets_updated');
            $messageType = 'success';
            break;
    }
}
...
$statusLabels = [
    'active' => ['label' => __('robot_status_running'), 'desc' => __('robot_status_running_desc'), 'icon' => 'fa-play-circle'],
    'paused' => ['label' => __('robot_status_paused'), 'desc' => __('robot_status_paused_desc'), 'icon' => 'fa-pause-circle'],
    'standby' => ['label' => __('robot_status_standby'), 'desc' => __('robot_status_standby_desc'), 'icon' => 'fa-clock'],
    'weekend' => ['label' => __('robot_status_weekend'), 'desc' => __('robot_status_weekend_desc'), 'icon' => 'fa-moon'],
    'inactive' => ['label' => __('robot_status_offline'), 'desc' => __('robot_status_offline_desc'), 'icon' => 'fa-power-off']
];
...
<div class="db-page-header">
    <div>
        <h1 class="db-page-title"><i class="fas fa-robot"></i> <?php _e('dashboard_title'); ?></h1>
        <p class="db-page-subtitle"><?php _e('dashboard_subtitle'); ?></p>
    </div>
    <div class="d-flex gap-2">
        <a href="statistics.php" class="db-btn db-btn-outline">
            <i class="fas fa-chart-bar"></i> <?php _e('dashboard_full_stats'); ?>
        </a>
    </div>
</div>
...
<div class="db-alert warning db-fade-in">
    ...
    <div class="flex-grow-1">
        <strong><?php _e('dashboard_ot_setup_required'); ?></strong>
        <p class="mb-0 small" style="opacity: 0.9;">
            <?php _e('dashboard_ot_setup_desc'); ?>
        </p>
    </div>
    <a href="olymptrade-setup.php" class="db-btn db-btn-warning db-btn-sm ms-3">
        <i class="fas fa-link"></i> <?php _e('dashboard_setup_now'); ?>
    </a>
</div>
<?php endif; ?>
...
<div class="db-alert warning db-fade-in">
    ...
    <div>
        <strong><?php _e('dashboard_weekend_mode'); ?></strong>
        <p class="mb-0 small" style="opacity: 0.9;"><?php _e('dashboard_weekend_desc'); ?></p>
    </div>
</div>
<?php endif; ?>
...
<div class="robot-status-panel db-fade-in">
    <div class="robot-status-header">
        ...
        <form method="POST" class="master-toggle">
            ...
            <label><?php _e('dashboard_master_switch'); ?></label>
            <?php if (!$otSetupCompleted): ?>
    ...
    <div class="row g-3 mt-2">
        <div class="col-md-6">
            <div class="db-progress-card success">
                <div class="db-progress-header">
                    <span><i class="fas fa-bullseye me-1"></i> <?php _e('dashboard_tp_target'); ?></span>
        ...
        <div class="col-md-6">
            <div class="db-progress-card danger">
                <div class="db-progress-header">
                    <span><i class="fas fa-shield-alt me-1"></i> <?php _e('dashboard_ml_limit'); ?></span>
                    <span class="<?php echo $todayPnl < 0 && abs($todayPnl) >= $mlLimit ? 'text-danger fw-bold' : ''; ?>">
                        ...
                        <?php else: ?>
                            <i class="fas fa-check text-success me-1"></i> <?php _e('dashboard_safe'); ?>
                        <?php endif; ?>
                    </span>
                </div>
                <div class="db-progress-bar">
...
<div class="db-stat-grid db-fade-in">
    <div class="db-stat-card">
        ...
        <div class="db-stat-label"><?php _e('dashboard_today_trades'); ?></div>
    </div>
    <div class="db-stat-card success">
        ...
        <div class="db-stat-label"><?php _e('dashboard_wins_today'); ?></div>
    </div>
    <div class="db-stat-card <?php echo ($todayPnl >= 0) ? 'success' : 'danger'; ?>">
        ...
        <div class="db-stat-label"><?php _e('dashboard_today_pl'); ?></div>
    </div>
    <div class="db-stat-card">
        ...
        <div class="db-stat-label"><?php _e('dashboard_winrate_7d'); ?></div>
    </div>
    <div class="db-stat-card <?php echo ($monthStats['total_pnl'] ?? 0) >= 0 ? 'success' : 'danger'; ?>">
        ...
        <div class="db-stat-label"><?php _e('dashboard_this_month'); ?></div>
    </div>
</div>
...
<div class="row g-4">
    ...
    <div class="col-lg-8">
        ...
        <div class="db-card db-fade-in mb-4">
            <div class="db-card-header">
                <h5 class="db-card-title"><i class="fas fa-chess"></i> <?php _e('dashboard_strategy_selection'); ?></h5>
                <span class="db-badge <?php echo strtolower($package); ?>"><?php echo strtoupper($package); ?> <?php _e('dashboard_access'); ?></span>
            </div>
                ...
                <form method="POST" id="settingsForm">
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            <div class="db-form-group">
                                <label class="db-form-label"><?php _e('dashboard_markets'); ?></label>
                        ...
                        <div class="col-md-6">
                            <div class="db-form-group">
                                <label class="db-form-label"><?php _e('dashboard_timeframes'); ?></label>
                                <div class="d-flex flex-wrap gap-2">
                                    <?php
                        ...
                        <div class="col-md-6">
                            <div class="db-form-group">
                                <label class="db-form-label"><?php _e('dashboard_trade_amount'); ?></label>
                        ...
                        <div class="col-md-6">
                            <div class="db-form-group">
                                <label class="db-form-label"><?php _e('dashboard_max_trades_day'); ?></label>
                    ...
                    <div class="mt-3">
                        <button type="submit" class="db-btn db-btn-primary">
                            <i class="fas fa-save"></i> <?php _e('dashboard_save_settings'); ?>
                        </button>
                    </div>
                </form>
            </div>
        ...
        <div class="db-card db-fade-in">
            <div class="db-card-header">
                <h5 class="db-card-title"><i class="fas fa-history"></i> <?php _e('dashboard_recent_trades'); ?></h5>
                <a href="statistics.php" class="db-btn db-btn-sm db-btn-outline"><?php _e('dashboard_view_all'); ?></a>
            </div>
            <div class="db-card-body" style="padding: 0;">
                <?php if (empty($recentTrades)): ?>
                <div class="text-center py-5">
                    <i class="fas fa-chart-line fa-3x text-muted mb-3" style="opacity: 0.3;"></i>
                    <p class="text-muted"><?php _e('dashboard_no_trades'); ?></p>
                </div>
                <?php else: ?>
                <div class="table-responsive">
                    <table class="db-table">
                        <thead>
                            <tr>
                                <th><?php _e('dashboard_time'); ?></th>
                                <th><?php _e('dashboard_asset'); ?></th>
                                <th><?php _e('dashboard_direction'); ?></th>
                                <th><?php _e('dashboard_amount'); ?></th>
                                <th><?php _e('dashboard_result'); ?></th>
                                <th><?php _e('dashboard_pl'); ?></th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <?php else: ?>
                                    <span class="db-badge warning"><?php _e('dashboard_pending'); ?></span>
                                    <?php endif; ?>
                                </td>
    ...
    <div class="col-lg-4">
        ...
        <div class="db-card db-fade-in mb-4">
            <div class="db-card-header">
                <h5 class="db-card-title"><i class="fas fa-bullseye"></i> <?php _e('dashboard_daily_targets'); ?></h5>
            </div>
            <div class="db-card-body">
                <form method="POST">
                    <input type="hidden" name="csrf_token" value="<?php echo generateCSRFToken(); ?>">
                    ...
                    <div class="db-form-group">
                        <label class="db-form-label"><?php _e('dashboard_take_profit'); ?></label>
                        <input type="number" name="daily_tp" class="db-form-control"
                               value="<?php echo $robotSettings['take_profit_target'] ?? 20; ?>" min="5" step="5">
                        <small class="text-muted"><?php _e('dashboard_auto_pause_desc_tp'); ?></small>
                    </div>

                    <div class="db-form-group">
                        <label class="db-form-label"><?php _e('dashboard_max_loss'); ?></label>
                        <input type="number" name="daily_ml" class="db-form-control"
                               value="<?php echo $robotSettings['max_loss_limit'] ?? 10; ?>" min="5" step="5">
                        <small class="text-muted"><?php _e('dashboard_auto_pause_desc_ml'); ?></small>
                    </div>
        ...
        <div class="db-card db-fade-in mb-4">
            <div class="db-card-header">
                <h5 class="db-card-title"><i class="fas fa-stream"></i> <?php _e('dashboard_live_activity'); ?></h5>
                <span class="db-badge info" id="liveIndicator">
                    <i class="fas fa-circle fa-xs"></i> <?php _e('dashboard_live'); ?>
                </span>
            </div>
            <div class="db-card-body" style="padding: 0;">
                <div class="db-live-log" id="liveLog">
                    ...
                    <div class="text-center py-4">
                        <p class="text-muted small mb-0"><?php _e('dashboard_no_activity'); ?></p>
                    </div>
                    <?php else: ?>
        ...
        <div class="db-card db-fade-in">
            <div class="db-card-header">
                <h5 class="db-card-title"><i class="fas fa-user-circle"></i> <?php _e('dashboard_account'); ?></h5>
            </div>
            <div class="db-card-body">
                ...
                <div class="d-flex justify-content-between mb-2">
                    <span class="text-muted"><?php _e('dashboard_expires'); ?></span>
                    <span><?php echo date('M d, Y', strtotime($user['package_expiry'])); ?></span>
                </div>
                <?php endif; ?>
                <div class="d-flex justify-content-between mb-2">
                    <span class="text-muted"><?php _e('dashboard_broker_id'); ?></span>
                    <code><?php echo htmlspecialchars($user['olymptrade_id']); ?></code>
                </div>
                <?php if (!empty($user['license_key'])): ?>
                <div class="d-flex justify-content-between mb-3">
                    <span class="text-muted"><?php _e('dashboard_license'); ?></span>
                ...
                <a href="pricing.php" class="db-btn db-btn-warning w-100">
                    <i class="fas fa-crown"></i> <?php _e('dashboard_upgrade_now'); ?>
                </a>
                <?php endif; ?>
            </div>
        </div>
