<?php
/**
 * ZYN Trade System - Registration Page
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif (!verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        if (empty($email) || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
            $errors[] = __('register_error_invalid_email');
        }

        if (strlen($password) < 8) {
            $errors[] = __('register_error_password_min');
        }
        ...
        if (empty($fullname)) {
            $errors[] = __('register_error_fullname_required');
        }

        if (empty($country)) {
            $errors[] = __('register_error_country_required');
        }

        if (empty($olymptrade_id) || strlen($olymptrade_id) < 6) {
            $errors[] = __('register_error_olymptrade_id');
        }

        // WhatsApp mandatory
        if (empty($phone)) {
            $errors[] = __('register_error_whatsapp_required');
        }

        if (!isset($_POST['terms'])) {
            $errors[] = __('register_error_terms_required');
        }

        if (!isset($_POST['affiliate'])) {
            $errors[] = __('register_error_affiliate_required');
        }
